...
def scrape_election_data_selenium(base_url, base_dir):
    ...
    polling_unit_count_by_location = {}
    pdf_count_by_location = {}
    ...
    for state_link in state_links:
        ...
        polling_unit_count_by_location.setdefault(state_name, {})
        pdf_count_by_location.setdefault(state_name, {})
        ...
        for lga_link in lga_links:
            ...
            polling_unit_count_by_location[state_name].setdefault(lga_name, {})
            pdf_count_by_location[state_name].setdefault(lga_name, {})
            ...
            for ward_link in ward_links:
                ...
                polling_unit_count_by_location[state_name][lga_name].setdefault(ward_name, 0)
                pdf_count_by_location[state_name][lga_name].setdefault(ward_name, 0)
                ...
                for pu_link, pu_name_div in zip(polling_unit_links, pu_names):
                    ...
                    polling_unit_count_by_location[state_name][lga_name][ward_name] += 1
                    ...
                    pdf_count_by_location[state_name][lga_name][ward_name] += 1
                ...
                # Add summary data for this ward
                summary_data.append({
                    'State': state_name,
                    'LGA': lga_name,
                    'Ward': ward_name,
                    'Total Polling Units': polling_unit_count_by_location[state_name][lga_name][ward_name],
                    'PDFs Downloaded': pdf_count_by_location[state_name][lga_name][ward_name],
                    'Non-PDF Files': non_pdf_files_by_location[state_name][lga_name][ward_name]['non_pdf'],
                    'Unreadable PDFs': unreadable_count_by_location[state_name][lga_name][ward_name]['unreadable']
                })
    ...
    print("Summary of polling units by location:")
    for state, lgas in polling_unit_count_by_location.items():
        for lga, wards in lgas.items():
            for ward, count in wards.items():
                print(f"{state} -> {lga} -> {ward}: {count} total polling units")

    print("Summary of PDFs by location:")
    for state, lgas in pdf_count_by_location.items():
        for lga, wards in lgas.items():
            for ward, count in wards.items():
                print(f"{state} -> {lga} -> {ward}: {count} PDFs downloaded")
...
if __name__ == "__main__":
    ...
    df_results, df_summary = scrape_election_data_selenium(BASE_URL, base_directory)
    print(df_results)
    print(df_summary)
